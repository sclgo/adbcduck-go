...
func TestNextRowTypes(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			arrowValueFunc: func(t *testing.T, b array.Builder) {
				t.Helper()
				b.(array.ListLikeBuilder).Append(true)
				bld := b.(array.ListLikeBuilder).ValueBuilder().(*array.Int32Builder)
				for i := range 5 {
					bld.Append(int32(i))
				}
			},
			golangValue: []driver.Value{int32(0), int32(1), int32(2), int32(3), int32(4)},
		},
	}
