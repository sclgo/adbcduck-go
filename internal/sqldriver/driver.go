...
// Forked from arrow-adbc project. See doc.go for details.
...
func (r *rows) Next(dest []driver.Value) error {
	...
	for i, col := range r.curRecord.Columns() {
		value, err := getValue(col, int(r.curRow))
		if err != nil {
			return err
		}
		dest[i] = value
...
func getValue(col arrow.Array, idx int) (driver.Value, error) {
	if col.IsNull(idx) {
		return nil, nil
	}
	...
	if colUnion, ok := col.(array.Union); ok {
		col = colUnion.Field(colUnion.ChildID(idx))
	}
	switch col := col.(type) {
	case *array.Boolean:
		value = col.Value(idx)
	...
	case *array.LargeBinary:
		value = col.Value(idx)
	case *array.Date32:
		value = col.Value(idx).ToTime()
	case *array.Date64:
		value = col.Value(idx).ToTime()
	case *array.Time32:
		value = col.Value(idx).ToTime(col.DataType().(*arrow.Time32Type).Unit)
	case *array.Time64:
		value = col.Value(idx).ToTime(col.DataType().(*arrow.Time64Type).Unit)
	...
	case array.ListLike:
		...
		var err error
		for ; i < j; i++ {
			listVal[i], err = getValue(col.ListValues(), int(i))
			if err != nil {
				return nil, err
			}
		}
